...
def zrot(phi: torch.tensor):
    """
    Calculate Bloch matrix for rotation around z axis with angle phi. 

    Args: 
        phi (tensor): rotation angle in rad
    ...
    Rz = torch.stack([torch.stack([cosphi, -sinphi, zero]),
...
def q_bloch(alpha: torch.tensor, phi: torch.tensor=torch.tensor(np.pi/2)):
    """
...
def r_bloch(t1: torch.tensor, t2: torch.tensor, t: torch.tensor, df: float=0):
    """
    ...
    Keyword Args:
        df (float): off-resonance frequency in Hz

    Returns:
        (tensor): relaxation matrix
    """

    phi = torch.tensor(2*np.pi*df*t/1000)
    E1 = torch.exp(-t/t1)
    E2 = torch.exp(-t/t2)

    M =  torch.stack([torch.stack([E2, zero, zero]),
...
def dr_dt1_bloch(t1: torch.tensor, t: torch.tensor):
    """
...
def dr_dt2_bloch(t2: torch.tensor, t: torch.tensor):
    """
...
def b_bloch(t1: torch.tensor, t: torch.tensor):
    """
...
def db_dt1_bloch(t1: torch.tensor, t: torch.tensor):
    """
...
def inversion_bloch(inv_eff: torch.tensor):
    """
...
def t2prep_bloch(t2: torch.tensor, t2te: torch.tensor):
    """
...
def dt2prep_dt2_bloch(t2: torch.tensor, t2te: torch.tensor):
    """
...
def calculate_signal_fisp_bloch(t1: float, t2: float, m0: float, beats: int, shots: int, fa: Union[np.ndarray, torch.tensor], tr: Union[np.ndarray, torch.tensor], ph: Union[np.ndarray, torch.tensor], prep: List[int], ti: List[int], t2te: List[int], te: float, inv_eff: float=1., delta_B1: float=1., n_iso: int=200, deph: int=2):
    """
    ...
    r_te = r_bloch(t1, t2, te)
    b_te = b_bloch(t1, te)
    inv = inversion_bloch(inv_eff)
    ...
    for ii in range(beats):
        ...
        if prep[ii] == 1:
            ...
            for iso in range(n_iso):
                m[iso] = r_bloch(t1, t2, ti[ii]) @ inv @ m[iso].clone() + m0/n_iso*b_bloch(t1, ti[ii])

        # T2 preparation
        elif prep[ii] == 2:
            # potentially inefficient implementation, might be accelerated by using batch matrix multiplication
            for iso in range(n_iso):
                m[iso] = t2prep_bloch(t2, t2te[ii]) @ m[iso].clone()
        ...
        for jj in range(shots):
            ...
            # Excitation matrix
            q_n = q_bloch(delta_B1*torch.deg2rad(fa[n]), torch.deg2rad(ph[n]))
            ...
            for iso in range(n_iso):                
                m[iso] = g(beta[iso]) @ r_bloch(t1, t2, tr[n]-te) @ m[iso].clone() + m0/n_iso*b_bloch(t1, tr[n]-te)
            
    return signal


def calculate_crlb_fisp_sc_bloch(t1, t2, m0, beats, shots, fa, tr, ph, prep, ti, t2te, te, inv_eff=1., delta_B1=1., n_iso=200, deph=2):
    """
    ...
    for ii in range(beats):
        ...
        if prep[ii] == 1:
            ...
            r_ti = r_bloch(t1, t2, ti[ii])

            for iso in range(n_iso):

                dm_dt1[iso] = dr_dt1_bloch(t1, ti[ii]) @ inv @ m[iso].clone() + r_ti @ inv @ dm_dt1[iso].clone() + m0/n_iso*db_dt1_bloch(t1, ti[ii])

                dm_dt2[iso] = dr_dt2_bloch(t2, ti[ii]) @ inv @ m[iso].clone() + r_ti @ inv @ dm_dt2[iso].clone()

                dm_dm0[iso] = r_ti @ inv @ dm_dm0[iso].clone()

                m[iso] = r_ti @ inv @ m[iso].clone() + m0/n_iso*b_bloch(t1, ti[ii])

        # T2 preparation
        elif prep[ii] == 2:
            
            t2prep_ii = t2prep_bloch(t2, t2te[ii])
            
            for iso in range(n_iso):

                dm_dt1[iso] = t2prep_ii @ dm_dt1[iso].clone()

                dm_dt2[iso] = dt2prep_dt2_bloch(t2, t2te[ii]) @ m[iso].clone() + t2prep_ii @ dm_dt2[iso].clone()
        ...
        for jj in range(shots):
            ...
            for iso in range(n_iso):
                ...
                dm_trans_dt2 = dm_trans_dt2 + P @ dr_dt2_bloch(t2, te) @ q_n @ m[iso].clone() + P @ r_te @ q_n @ dm_dt2[iso].clone()
            ...
            r_tr = r_bloch(t1, t2, tr[n])
            b_tr = b_bloch(t1, tr[n])

            for iso in range(n_iso):

                g_temp = g(beta[iso])

                dm_dt1[iso] = g_temp @ dr_dt1_bloch(t1, tr[n]) @ q_n @ m[iso].clone() + g_temp @ r_tr @ q_n @ dm_dt1[iso].clone() + m0/n_iso*db_dt1_bloch(t1, tr[n])

                dm_dt2[iso] = g_temp @ dr_dt2_bloch(t2, tr[n]) @ q_n @ m[iso].clone() + g_temp @ r_tr @ q_n @ dm_dt2[iso].clone()
...
def calculate_crlb_fisp_mc_bloch(t1: List[float], t2: List[float], m0: float, ratio: float, beats: int, shots: int, fa: Union[np.ndarray, torch.tensor], tr: Union[np.ndarray, torch.tensor], ph: Union[np.ndarray, torch.tensor], prep: List[int], ti: List[int], t2te: List[int], te: float, inv_eff: float=1., delta_B1: float=1., n_iso: int=200, deph: int=2):
    """
    ...
    r_te_1 = r_bloch(t1[0], t2[0], te)
    r_te_2 = r_bloch(t1[1], t2[1], te)
    inv = inversion_bloch(inv_eff)
    ...
    for ii in range(beats):
        ...
        if prep[ii] == 1:
            ...
            r_ti_1 = r_bloch(t1[0], t2[0], ti[ii])
            r_ti_2 = r_bloch(t1[1], t2[1], ti[ii])
            b_ti_1 = b_bloch(t1[0], ti[ii])
            b_ti_2 = b_bloch(t1[1], ti[ii])
        ...
        # T2 preparation
            ...
            t2prep_ii_1 = t2prep_bloch(t2[0], t2te[ii])
            t2prep_ii_2 = t2prep_bloch(t2[1], t2te[ii])
        ...
        for jj in range(shots):
            ...
            r_tr_1 = r_bloch(t1[0], t2[0], tr[n])
            r_tr_2 = r_bloch(t1[1], t2[1], tr[n])
            b_tr_1 = b_bloch(t1[0], tr[n])
            b_tr_2 = b_bloch(t1[1], tr[n])
...
def calculate_orth_bloch(t1: List[float], t2: List[float], beats: int, shots: int, fa: Union[np.ndarray, torch.tensor], tr: Union[np.ndarray, torch.tensor], ph: Union[np.ndarray, torch.tensor], prep: List[int], ti: List[int], t2te: List[int], te: float, inv_eff: float=1., delta_B1: float=1., n_iso: int=200, deph: int=2):
    """
    ...
    for ii in range(n_tissues):
        ...
        s[ii] = calculate_signal_fisp_bloch(t1[ii], t2[ii], 1, beats, shots, fa, tr, ph, prep, ti, t2te, te, inv_eff, delta_B1, n_iso, deph)
...
def calculate_signal_bssfp_bloch(t1: float, t2: float, m0: float, beats: int, shots: int, fa: Union[np.ndarray, torch.tensor], tr: Union[np.ndarray, torch.tensor], ph: Union[np.ndarray, torch.tensor], prep: List[int], ti: List[int], t2te: List[int], te: float, inv_eff: float=1., delta_B1: float=1., df: float=0):
    """
    Function to calculate the resulting complex signal of a voxel for a given MRF sequence using EPG.
    ...
    # Calculate relaxation matrix, longitudinal relaxation term, and inversion matrix
    r_te = r_bloch(t1, t2, te, df)
    b_te = b_bloch(t1, te)
    inv = inversion_bloch(inv_eff)

    # Initial magnetization
    m = torch.tensor([[0], [0], [m0]], dtype=torch.double)
    ...
    for ii in range(beats):
        ...
        # Inversion preparation
        if prep[ii] == 1:
            m = r_bloch(t1, t2, ti[ii], df) @ inv @ m.clone() + m0*b_bloch(t1, ti[ii])

        # T2 preparation
        elif prep[ii] == 2:
            m = t2prep_bloch(t2, t2te[ii]) @ m.clone()
        ...
        for jj in range(shots):
            ...
            # Update magnetization (relaxation during TR-TE, gradient dephasing)
            m = r_bloch(t1, t2, tr[n]-te, df) @ m.clone() + m0*b_bloch(t1, tr[n]-te)
